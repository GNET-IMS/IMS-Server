...
exports.find = (token) => {
  try {
    ...
    return AccessToken.findOne({ value: id }).exec();
  } catch (error) {
...
exports.save = (token, expirationDate, userID, clientID, scope) => {
  ...
  return accessToken.save((err, doc) => {
...
exports.delete = (token) => {
  try {
    const id = jwt.decode(token).jti;
    return AccessToken.remove({value: id}).exec();
  } catch (error) {
    return Promise.resolve(undefined);
  }
};
...
exports.removeExpired = () => {
  return AccessToken.remove({expiration: {'$lt': new Date()}}).exec();
};
 ...
 */
exports.removeAll = () => {
  return AccessToken.collection.remove().exec();
};
